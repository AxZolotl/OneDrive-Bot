...
class DriveItems():
    ...
    def search_folder(self, search_query: str) -> dict:
        content = self.graph_session.make_request(
            method="get",
            endpoint="me/" + self.endpoint + f"/root/search(q='{search_query}')"
        )
        
        return content
    
    def create_sl(self, item_id: str, request_body: dict) -> dict:
        add_headers={
            "Content-Type": "application/json"
        }
        content = self.graph_session.make_request(
            method="post",
            endpoint="me/" + self.endpoint + f"/items/{item_id}/createLink",
